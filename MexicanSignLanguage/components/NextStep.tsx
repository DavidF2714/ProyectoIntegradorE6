...
export default function SpellingBee() {
  ...
  useEffect(() => {
    const fetchPredictions = async () => {
      ...
      try {
        const res = await fetch("http://172.16.30.26:8000/user_predictions/", {
          headers: {
        ...
        const enriched = await Promise.all(
          data.predictions.map(async (pred: any) => {
            const frameRes = await fetch(`http://172.16.30.26:8000/get_frames/${pred.word_id}`, {
  ...
  useEffect(() => {
    ...
    const initWebSocket = () => {
      socketRef.current = new WebSocket("ws://172.16.30.26:8000/ws");
  ...
  const captureAndSaveFrame = async (letter: string) => {
    ...
    try {
      await fetch(`http://172.16.30.26:8000/save_frame/${letter}`, {
        method: "POST",
        body: form,
        headers: {
          "Authorization": `Bearer ${localStorage.getItem("token") || ""}`,
  ...
  async function getSpelledWord(wordId: string, token: string) {
    const res = await fetch(`http://172.16.30.26:8000/get_frames/${wordId}`, {
      headers: {
    ...
    const timeout = setTimeout(() => {
      getSpelledWord(wordUUIDRef.current, token)
        .then(frames => {
          ...
          fetch("http://172.16.30.26:8000/save_prediction/", {
            method: "POST",
            headers: {
              "Authorization": `Bearer ${token}`,
            },
